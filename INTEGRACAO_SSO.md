...
| `role` | enum | `SUPER_ADMIN`, `COMPANY_ADMIN`, `COMPANY_SUPERVISOR` ou `COMPANY_OPERATOR` |
...
app.get('/perfil', requireAuth, (req, res) => {
  ...
  // user.role     -> Role global no SignOn (SUPER_ADMIN | COMPANY_ADMIN | COMPANY_SUPERVISOR | COMPANY_OPERATOR)
  // user.appRole  -> Role especifica para esta aplicacao (COMPANY_OPERATOR | COMPANY_SUPERVISOR)
...
### 11.1 Role Global (`role`)

O campo `role` representa o papel do usuario na plataforma SignOn:

| Role | Descricao |
|------|-----------|
| `SUPER_ADMIN` | Administrador global do SignOn |
| `COMPANY_ADMIN` | Administrador da empresa |
| `COMPANY_SUPERVISOR` | Supervisor da empresa |
| `COMPANY_OPERATOR` | Operador da empresa |

### 11.2 Role por Aplicacao (`appRole`)

O campo `appRole` representa o papel do usuario **especificamente para a aplicacao que esta sendo acessada**. Um mesmo usuario pode ter roles diferentes em aplicacoes diferentes.

Por exemplo, um usuario pode ser `COMPANY_OPERATOR` no App A e `COMPANY_SUPERVISOR` no App B. Essa configuracao e feita pelo administrador da empresa no painel de Equipe do SignOn.

Os valores possiveis para `appRole` sao:

| appRole | Descricao |
|---------|-----------|
| `COMPANY_OPERATOR` | Operador (padrao ao vincular usuario a aplicacao) |
| `COMPANY_SUPERVISOR` | Supervisor |

> **Recomendacao**: Para controle de acesso dentro da aplicacao externa, use o campo `appRole` em vez de `role`. O `appRole` reflete o nivel de acesso que o administrador da empresa definiu para aquele usuario naquela aplicacao especifica.

### 11.3 Mapeamento de Roles

A aplicacao externa pode mapear estes roles para seus proprios niveis de acesso:

```typescript
function getLocalRole(appRole: string): string {
  switch (appRole) {
