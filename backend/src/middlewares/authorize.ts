...
export function canManageUser(
  ...
  // COMPANY_ADMIN so pode gerenciar COMPANY_OPERATOR e COMPANY_SUPERVISOR da sua companhia
  if (req.user.role === 'COMPANY_ADMIN') {
    if (targetRole && targetRole !== 'COMPANY_OPERATOR' && targetRole !== 'COMPANY_SUPERVISOR') {
      throw new AppError(
        'Voce so pode gerenciar operadores e supervisores',
        403,
        'CANNOT_MANAGE_ROLE'
      );
    }
  ...
  // COMPANY_OPERATOR e COMPANY_SUPERVISOR nao podem gerenciar usuarios
  res.status(403).json({
    success: false,
