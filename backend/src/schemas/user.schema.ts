...
export const createUserSchema = z.object({
  ...
  (data) => {
    ...
    // COMPANY_ADMIN, COMPANY_SUPERVISOR e COMPANY_OPERATOR devem ter companyId
    if ((data.role === 'COMPANY_ADMIN' || data.role === 'COMPANY_SUPERVISOR' || data.role === 'COMPANY_OPERATOR') && !data.companyId) {
      return false;
    }
    return true;
  },
  {
    message: 'SUPER_ADMIN nao deve ter companhia. COMPANY_ADMIN, COMPANY_SUPERVISOR e COMPANY_OPERATOR devem ter companhia.',
  }
);
