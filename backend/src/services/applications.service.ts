...
export class ApplicationsService {
  ...
  async getAvailableForUser(userId: string, companyId: string | null, role: string) {
    ...
    if (role === 'COMPANY_OPERATOR' || role === 'COMPANY_SUPERVISOR') {
      // Operador/Supervisor - apenas aplicacoes atribuidas individualmente (que tambem sao contratadas pela empresa)
  ...
  async generateAccessUrl(applicationId: string, userId: string, companyId: string | null, role: string) {
    ...
    if (companyId) {
      ...
      // Para operadores e supervisores, verificar tambem a atribuicao individual
      if (role === 'COMPANY_OPERATOR' || role === 'COMPANY_SUPERVISOR') {
